...
import {favThread, hideThread} from "../helpers/thread-management.ts";
...
const Board:React.FC<PageData> = ({isMobile}) => {
    ...
    const [favedThreads, setFavedThreads] = useState<string[]>(() => {
        const storedThreads = localStorage.getItem('favedThreads');
        return storedThreads ? storedThreads.split(',') : [];
    });
    ...
    const handleToggleFav = (threadNumber: string) => {
        favThread(threadNumber, setFavedThreads);
    };
                    ...
                    <div className="w-full h-full bg-white dark:bg-gray-900 ">
                        <div className="gap-4">
                            ...
                            {data?.threads?.map((thread) => (
                                <ThreadCard onToggleHide={handleToggleHide}
                                            onToggleFav={handleToggleFav}
                                            threadIsFaved={(favedThreads?.find(t => t === thread.num.toString()) ?? []).length > 0}
                                            threadIsHidden={(hiddenThreads?.find(t => t === thread.num.toString()) ?? []).length > 0}
                                            isMobile={isMobile}
                                            key={thread.num}
                                            nsfwMode={nsfwMode}
                                            thread={thread}/>
